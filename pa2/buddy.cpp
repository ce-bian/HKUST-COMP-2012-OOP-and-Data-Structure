...
bool Buddy::capable(Activity<Workout> workout_activity)
{
	 ...
	 * Add your code here!
	 * */
	if((workout_activity.item.getEnergyChange()+energy<0)||
				(workout_activity.item.getFatChange()+fat<0)||(workout_activity.item.getMuscleChange()+muscle<0))
			return false;

	else if(!workout_schedule.isFree(workout_activity.start_time,workout_activity.end_time))
			  return false;
	  else
		return true;
}
...
bool Buddy::capable(Activity<Dining> dining_activity) {
	...
	if(money-dining_activity.item.getPrice()<0)
		...
		if(!dining_schedule.isFree(dining_activity.start_time,dining_activity.end_time))
			return false;
...
Buddy& Buddy::operator<<(Activity<Dining> dining_activity) {
	...
	money-=dining_activity.item.getPrice();
	}
	return *this;
}
